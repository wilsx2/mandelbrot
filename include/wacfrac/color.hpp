...
SYCL_EXTERNAL
inline auto colorize_discrete(unsigned n, unsigned max_n, sycl::accessor<Pixel, 1, sycl::access_mode::read> palette) -> Pixel {
    if (n == max_n)
...
SYCL_EXTERNAL
inline auto colorize_continuous(Complex<float> z, unsigned n, unsigned max_n, sycl::accessor<Pixel, 1, sycl::access_mode::read> palette) -> Pixel
{
    using namespace std;
    using namespace std;
    if (n == max_n)
