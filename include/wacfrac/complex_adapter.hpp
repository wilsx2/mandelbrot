...
class Complex {
    ...
    public:
    SYCL_EXTERNAL Complex() : _real(0), _imag(0) {}
    SYCL_EXTERNAL Complex(const T& n) : _real(n), _imag(0) {}
    template<typename U>
        requires (std::is_arithmetic_v<U> && !std::same_as<std::remove_cvref_t<U>, T>)
    SYCL_EXTERNAL Complex(U n) : _real(static_cast<T>(n)), _imag(0) {}
    SYCL_EXTERNAL Complex(const T& r, const T& i) : _real(r), _imag(i) {}
