...
namespace wacfrac::bla
{
    ...
    template <ComplexConcept T> struct Approximator
    {
        ...
        {
            return level >= first_level && m > 0 && m - 1 < columns.size() &&
                   level - first_level < columns[m - 1].count;
        }
        SYCL_EXTERNAL
        ...
        auto approximate_dzn(T dzm, unsigned m, T dc) const -> std::optional<std::pair<T, unsigned>>
        {
            ...
            auto n = m + 1;
            for (auto level = static_cast<std::ptrdiff_t>(last_level);
                 level >= static_cast<std::ptrdiff_t>(first_level); --level)
            {
    ...
    public:
        ...
        auto compute_search(std::span<const T> probes, T max_dc, std::span<const T> ref, double escape_radius = 2.0)
            -> void
        {
    ...
    private:
        ...
        auto compute_initial_approximations(CT epsilon, std::span<const T> ref, T max_dc) -> void
        {
            ...
            _queue
                .parallel_for(ref.size() - 2,
                              [=](sycl::id<1> id) {
                                  auto m{id + 1};
                                  Bla<T> bla{epsilon, ref, max_dc, static_cast<unsigned>(m),
        ...
        auto compute_probe_escape_time(std::span<const T> probes, std::span<const T> ref, double escape_radius) -> void
        {
            ...
            _queue
                .parallel_for(probes.size(),
                              [=](sycl::id<1> id) {
                                  escape_times[id] =
                                      escape_perturbed<T>(probes[id], ref, static_cast<unsigned>(ref.size()),
                                                          escape_radius)
                                          .second;
                              })
                .wait();
