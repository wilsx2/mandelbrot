...
template <typename Real> auto to_real(const MultiFloat& val) -> Real
{
...
template <typename Real, typename Number> auto to_real(const Number& val) -> Real
{
    using NumberDoubleExp = boost::multiprecision::number<DoubleExp>;
...
template <ComplexConcept T> auto to_complex(MultiComplex z) -> T
{
    using CT = ComplexValueTypeT<T>;
    return T{to_real<CT>(boost::multiprecision::real(z)), to_real<CT>(boost::multiprecision::imag(z))};
}

template <> inline auto to_complex<MultiComplex>(MultiComplex z) -> MultiComplex
{
    return z;
}
