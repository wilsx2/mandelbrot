...
namespace wacfrac
{
    ...
    struct Viewport
    {
        ...
        auto required_iterations(double modifier = 250.0, double factor = 50.0, double exponent = 1.5) const
            -> unsigned;
        template <ComplexConcept T> auto get_corner_absolute() const -> T;
        template <ComplexConcept T> auto get_corner_relative() const -> T;
    ...
    auto required_iterations(MultiFloat zoom_factor, double modifier = 250.0, double factor = 50.0,
                             double exponent = 1.5) -> unsigned;

    template <ComplexConcept T> auto Viewport::get_corner_absolute() const -> T
    {
        using CT = ComplexValueTypeT<T>;
        return {to_real<CT>(center.real()) - to_real<CT>(dimensions.real()) / static_cast<CT>(2.0),
                to_real<CT>(center.imag()) - to_real<CT>(dimensions.imag()) / static_cast<CT>(2.0)};
    }
    template <ComplexConcept T> auto Viewport::get_corner_relative() const -> T
    {
        using CT = ComplexValueTypeT<T>;
        return {-to_real<CT>(dimensions.real()) / static_cast<CT>(2.0),
    ...
    auto Viewport::generate_probes(sycl::queue& q, std::span<T> probes, sycl::range<2> range) const -> void
    {
        ...
        q.parallel_for(range, [=](sycl::id<2> id) {
             probes[id[0] + id[1] * cols] = T{corner.real() + delta.real() * static_cast<CT>(id[1]),
