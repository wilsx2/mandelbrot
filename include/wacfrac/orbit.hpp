...
auto escape_generic(T z0, std::size_t n, std::size_t max_n, F&& next_z) -> std::pair<std::complex<double>, std::size_t> {
    auto z {z0};
...
template<Complex T>
auto escape(T c, std::size_t max_n) -> std::pair<std::complex<double>, std::size_t> {
...
auto escape_perturbed(const std::vector<std::complex<double>>& ref, std::complex<double> dc, std::size_t max_n, std::complex<double> dz = {0.0, 0.0}, std::size_t n = 0) -> std::pair<std::complex<double>, std::size_t>;

auto compute_reference(multi_complex c, std::size_t max_n, bool do_escape = true) -> std::vector<std::complex<double>>; // TODO: Check if it is possible no references should escape
