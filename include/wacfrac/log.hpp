...
enum class Severity { Trace = 0, Debug = 1, Info = 2, Warning = 3, Error = 4, Fatal = 5 };
...
template <typename... Args> inline void print(Severity level, std::string_view fmt, Args&&... args)
{
    ...
    auto label = static_cast<std::size_t>(level) < 6 ? strings[static_cast<std::size_t>(level)]
...
template <typename... Args> inline void warning(std::string_view fmt, Args&&... args)
{
...
template <typename... Args> inline void fatal(std::string_view fmt, Args&&... args)
{
