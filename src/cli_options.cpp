...
auto parse_numeric_type(const std::string& value) -> NumericType {
    if (value == "auto")  return NumericType::Auto;
    if (value == "float") return NumericType::Float;
    if (value == "double") return NumericType::Double;
    if (value == "dexp")  return NumericType::DoubleExp;
...
auto parse_render_type(const std::string& value) -> RenderType {
    if (value == "auto") return RenderType::Auto;
    if (value == "direct") return RenderType::Direct;
    if (value == "perturbed") return RenderType::Perturbed;
    if (value == "bla") return RenderType::BLA;
...
auto parse_arguments(int argc, char* argv[]) -> std::optional<CliOptions> {
    ...
    auto add_renderer_params = [&opts](argparse::ArgumentParser& p) {
        ...
        p.add_argument("--first-level", "-L")
            .action([](const std::string& v) { return static_cast<std::size_t>(std::stoul(v)); })
    ...
    // Image params
    image_cmd.add_argument("--output", "-o")
        .store_into(opts.image.filepath)
    ...
    if (program.is_subcommand_used(image_cmd)) {
        ...
        std::cerr << "No render mode specified. Use either image or video\n";
        std::cerr << program;
        return std::nullopt;
    }
