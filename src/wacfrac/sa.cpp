...
series_approximator::series_approximator(const std::vector<std::complex<long double>>& reference_orbit, std::size_t coefficient_count)
...
void series_approximator::compute_next_coeffs() {
    for (auto&& c : std::views::iota(0uz, _next_coeffs.size())) {
        _next_coeffs[c] = 2.0L * _reference_orbit[_n] * _curr_coeffs[c];
        if (c == 0) {
            _next_coeffs[c] += 1.0L;
            continue;
        }
        for (auto&& c1 : std::views::iota(0uz, c)) {
            auto c2 = c - c1 - 1;
            _next_coeffs[c] += (c1 != c2 ? 2.0L : 1.0L) * _curr_coeffs[c1] * _curr_coeffs[c2];
        }
    }
}
...
void series_approximator::compute_coeffs_while_valid(const std::vector<std::complex<long double>>& deltas, double threshold) {
...
auto series_approximator::approximate_delta_n(std::complex<long double> delta_0) const -> std::complex<long double> {
    std::complex<long double> z_n {0.0, 0.0};
    std::complex<long double> c_pow {1.0, 0.0};
...
auto series_approximator::is_valid(std::complex<long double> delta_0, double threshold) const -> bool {
    std::complex<long double> c_pow {1.0, 0.0};
    std::complex<long double> prev_term {0.0, 0.0};
