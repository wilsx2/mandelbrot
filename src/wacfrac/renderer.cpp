...
namespace wacfrac
{
    ...
    template <typename T> struct NumericTypeTag
    {
        using type = T;
    };

    template <typename F> decltype(auto) with_numeric_type(NumericType type, F&& f)
    {
        switch (type)
        {
                            ...
                            std::pair<std::size_t, std::size_t> probe_grid) -> std::size_t
    {
        ...
        {
            auto last_level =
                max_n < 3 ? std::size_t{0} : static_cast<std::size_t>(std::log2(static_cast<double>(max_n)));
            auto n = max_n - 1;
    ...
    auto Renderer::render_pass(T start, T delta, unsigned max_n, F pixel_escape) -> void
    {
        ...
        conf.queue
            .parallel_for(conf.resolution.range(),
                          [=](sycl::id<2> id) {
                              auto [z, n] = pixel_escape(id);
          ...
          arena{conf.queue, 0}
    {
        ...
        auto hint_max_n = required_iterations(MultiFloat(10.0), conf.iteration_parameters.modifier,
                                              conf.iteration_parameters.factor, conf.iteration_parameters.exponent);
        auto initial_size = compute_arena_size<DoubleExpComplex>(static_cast<unsigned>(hint_max_n), RenderType::BLA,
                                                                 conf.bla_config, conf.probe_grid);
        if (initial_size > 0)
        ...
        logging::info("Renderer Config: device={} resolution={}x{} focus={}x{} escape_radius={} palette size={}\
                    ...
                    epsilon_range=10^{}-10^{} epsilon_tolerance={} epsilon_convergence_rad={}",
                      conf.queue.get_device().get_info<sycl::info::device::name>(), conf.resolution.width,
                      conf.resolution.height, conf.focus.real(), conf.focus.imag(), conf.escape_radius,
                      conf.palette.as_span().size(), conf.discrete_coloring, conf.iteration_parameters.modifier,
                      conf.iteration_parameters.factor, conf.iteration_parameters.exponent, conf.bla_config.first_level,
    ...
    auto Renderer::render(const ImageConfig& img_conf) -> std::span<const Pixel>
    {
        ...
        logging::info(
            "Render Config: zoom={} max_iterations={} precision={} numeric_type={} render_type={}", img_conf.scale,
            max_n, precision,
            [&]() {
                switch (num_type)
                {
        ...
        with_numeric_type(num_type, [&]<typename T>(NumericTypeTag<T>) {
            ...
            if (render_type == RenderType::Direct)
            {
                ...
                render_pass(start, delta, max_n, [=](sycl::id<2> id) {
                    return escape(sample_c_value(id, start, delta), max_n, esc_rad);
                });
            }
            else
            {
                auto start{view.get_corner_relative<T>()};
                logging::debug("start, relative: ({}, {})", static_cast<double>(start.real()),
                               static_cast<double>(start.imag()));
                ...
                else if (render_type == RenderType::BLA)
                {
                    ...
                    else
                    {
                        ...
                        view.generate_probes<T>(conf.queue, probes,
                                                sycl::range(conf.probe_grid.first, conf.probe_grid.second));
                        bla_calculator.compute_search(probes, max_dc, ref, conf.escape_radius);
                    }
                    auto bla = bla_calculator.get_approximator();
                    render_pass(start, delta, max_n, [=](sycl::id<2> id) {
                        auto [z, n, _] =
                            bla::escape_approximate(sample_c_value(id, start, delta), ref, max_n, esc_rad, bla);
                        return std::pair{z, n};
                    });
                }
