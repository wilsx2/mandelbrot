...
namespace wacfrac
{
    ...
    auto concatenate_images(std::filesystem::path output, std::string_view pattern, float fps) -> bool
    {
        ...
        return std::system(
                   std::format("ffmpeg -y -framerate {} -i {} -c:v libx264 -pix_fmt yuv420p {} > /dev/null 2>&1", fps,
    ...
    auto concatenate_videos(std::filesystem::path output, std::string_view pattern) -> bool
    {
        ...
        auto tmp =
            std::filesystem::temp_directory_path() / std::format("wacfrac_concat_{}.txt", std::random_device{}());
