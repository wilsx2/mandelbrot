...
if [[ $(gum choose --header="Render Type: " image video) == "image" ]]; then
    ...
    cmd="../build/wacfrac -f $(cat $poi) -c \"$(cat $palette)\" -r $resolution image -z $zoom -o $output"
else
    poi=$(gum file --padding="1 0 0 0" --header="Focus Point:" ./poi)
    ...
    cmd="../build/wacfrac -f $(cat $poi) -c \"$(cat $palette)\" -r $resolution video -a $initial -b $final -z $zps --fps $fps -o $output"
fi
