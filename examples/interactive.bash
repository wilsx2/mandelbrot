...
if [[ $(gum choose --header="Render Type: " image video) == "image" ]]; then
    ...
    cmd="../build/wacfrac auto -f $(cat $poi) -z $zoom -c \"$(cat $palette)\" -r $resolution -o $output -s -e 4"
else
    poi=$(gum file --padding="1 0 0 0" --header="Focus Point:" ./poi)
    ...
    cmd="../build/wacfrac video -f $(cat $poi) -a $initial -b $final -z $zps --fps $fps -c \"$(cat $palette)\" -r $resolution -o $output -s -e 4"
fi
